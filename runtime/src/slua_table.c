...
SluaTable* slua_table_new(void) {
    ...
    t->array_size = 0;
    /* hash_part starts NULL — allocated on first hash key insertion */
    return t;
}
...
/* Array part: integer keys 1..N. Hash part: everything else. */
...
static uint32_t hash_key(SluaValue key) {
    switch (key.tag) {
        ...
        case SLUA_TAG_STRING:{
            SluaString* s = (SluaString*)key.val.ptr;
            uint32_t h = 2166136261u;
            for (int i = 0; i < s->len; i++) h = (h ^ (uint8_t)s->data[i]) * 16777619u;
            return h;
        }
...
SluaValue slua_table_get(SluaTable* t, SluaValue key) {
    ...
    do {
        if (slua_equal(node->key, key)) return node->val;
        node = node->next;
    } while (node);
    return slua_null();
}
...
void slua_table_set(SluaTable* t, SluaValue key, SluaValue val) {
    ...
    do {
        if (slua_equal(cur->key, key)) { cur->val = val; return; }
        if (!cur->next) break;
        cur = cur->next;
    } while (1);
