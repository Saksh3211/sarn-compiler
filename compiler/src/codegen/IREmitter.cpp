...
void IREmitter::declare_runtime() {
    ...
    declare("slua_num_to_str", i8p, {llvm::Type::getDoubleTy(ctx_)});
    declare("slua_i64_to_str", i8p, {i64});
    ...
    if (e.op == "..") {
        ...
        auto to_str = [&](llvm::Value* v) -> llvm::Value* {
            ...
            if (v->getType()->isDoubleTy()) {
                auto* cvt = get_runtime_fn("slua_num_to_str");
                if (cvt) return builder_.CreateCall(cvt, {v}, "numstr");
            }
            if (v->getType()->isIntegerTy()) {
                auto* i64t = llvm::Type::getInt64Ty(ctx_);
                auto* ext = builder_.CreateSExt(v, i64t);
                auto* cvt = get_runtime_fn("slua_i64_to_str");
                if (cvt) return builder_.CreateCall(cvt, {ext}, "intstr");
            }
            return v;
        };
        lhs = to_str(lhs);
