...
double sarn_sqrt(double x) { return sqrt(x); }
double sarn_pow(double b, double e) { return pow(b, e); }
double sarn_sin(double x) { return sin(x); }
double sarn_cos(double x) { return cos(x); }
double sarn_tan(double x) { return tan(x); }
double sarn_log(double x) { return log(x); }
double sarn_log2(double x) { return log2(x); }
double sarn_exp(double x) { return exp(x); }
double sarn_inf(void) { return HUGE_VAL; }
double sarn_nan(void) { return nan(""); }
double sarn_pi(void) { return 3.14159265358979323846; }
double sarn_e(void) { return 2.71828182845904523536; }
...
static SarnValue box_str(const char* s) {
    SarnValue v;
    memset(&v, 0, sizeof(v));
...
int64_t sarn_tbl_iget_i64(SarnTable* t, int64_t key) {
    SarnValue v = sarn_table_get(t, sarn_int(key));
    return (v.tag == SARN_TAG_FLOAT) ? (int64_t)v.val.fval : v.val.ival;
}

double sarn_tbl_iget_f64(SarnTable* t, int64_t key) {
    SarnValue v = sarn_table_get(t, sarn_int(key));
    return (v.tag == SARN_TAG_INT) ? (double)v.val.ival : v.val.fval;
}

const char* sarn_tbl_iget_str(SarnTable* t, int64_t key) {
    SarnValue v = sarn_table_get(t, sarn_int(key));

    if (v.tag != SARN_TAG_STRING || !v.val.ptr)
        return "";
...
int64_t sarn_tbl_sget_i64(SarnTable* t, const char* key) {
    SarnValue v = sarn_table_get(t, str_key(key));
    return (v.tag == SARN_TAG_FLOAT) ? (int64_t)v.val.fval : v.val.ival;
}
