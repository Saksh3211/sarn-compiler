...
char* sarn_json_encode_str(const char* s)   { return json_esc(s?s:""); }
char* sarn_json_encode_int(int64_t n)       { char* r=(char*)malloc(32); snprintf(r,32,"%lld",(long long)n); return r; }
char* sarn_json_encode_float(double f)      { char* r=(char*)malloc(64); snprintf(r,64,"%g",f); return r; }
char* sarn_json_encode_bool(int32_t b)      { return strdup(b?"true":"false"); }
char* sarn_json_encode_null(void)           { return strdup("null"); }
...
int64_t sarn_json_get_int(const char* json, const char* key)   { const char* v=_find_key(json,key); return v?(int64_t)strtoll(v,NULL,10):0; }
double  sarn_json_get_float(const char* json, const char* key) { const char* v=_find_key(json,key); return v?strtod(v,NULL):0.0; }
int32_t sarn_json_get_bool(const char* json, const char* key)  { const char* v=_find_key(json,key); return v?(*v=='t'):0; }
int32_t sarn_json_has_key(const char* json, const char* key)   { return _find_key(json,key)?1:0; }
char*   sarn_json_minify(const char* json)                     { return strdup(json?json:""); }
...
int64_t sarn_json_get_nested_int(const char* json, const char* outer, const char* inner) {
    return (int64_t)sarn_json_get_nested_float(json, outer, inner);
}
