...
char*   sarn_http_get(const char* url)  { return _winhttp_request(url,"GET",NULL,NULL,NULL); }
char*   sarn_http_post(const char* url, const char* body, const char* ctype) { return _winhttp_request(url,"POST",body,ctype,NULL); }
char*   sarn_http_post_json(const char* url, const char* body) { return _winhttp_request(url,"POST",body,"application/json",NULL); }
int32_t sarn_http_status(const char* url) { int s=0; char* r=_winhttp_request(url,"GET",NULL,NULL,&s); free(r); return s; }
char*   sarn_http_get_header(const char* url, const char* hdr) { (void)url;(void)hdr; return strdup(""); }
#else
...
char*   sarn_http_get(const char* url)  { return _sock_request(url,"GET",NULL,NULL); }
char*   sarn_http_post(const char* url, const char* body, const char* ctype) { return _sock_request(url,"POST",body,ctype); }
char*   sarn_http_post_json(const char* url, const char* body) { return _sock_request(url,"POST",body,"application/json"); }
int32_t sarn_http_status(const char* url) { char* r=sarn_http_get(url); int ok=(r&&r[0])?200:0; free(r); return ok; }
char*   sarn_http_get_header(const char* url, const char* hdr) { (void)url;(void)hdr; return strdup(""); }
#endif
