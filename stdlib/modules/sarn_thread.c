...
#include <windows.h>
typedef struct { SarnThreadFn fn; void* arg; } ThCtx;
...
void    sarn_thread_sleep_ms(int64_t ms) { Sleep((DWORD)ms); }
int64_t sarn_thread_self_id(void) { return (int64_t)GetCurrentThreadId(); }
#else
#include <pthread.h>
#include <unistd.h>
typedef struct { SarnThreadFn fn; void* arg; } ThCtx;
...
void    sarn_thread_sleep_ms(int64_t ms) { usleep((useconds_t)(ms*1000)); }
int64_t sarn_thread_self_id(void) { return (int64_t)(uintptr_t)pthread_self(); }
#endif
