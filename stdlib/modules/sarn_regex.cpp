...
#include <iterator>
extern "C" {
int32_t sarn_regex_match(const char* str, const char* pat) {
    try { return std::regex_search(std::string(str), std::regex(pat)) ? 1 : 0; } catch (...) { return 0; }
}
int32_t sarn_regex_find(const char* str, const char* pat, int32_t from) {
    try {
...
char* sarn_regex_replace(const char* str, const char* pat, const char* repl) {
    try {
...
char* sarn_regex_groups(const char* str, const char* pat) {
    try {
