...
int32_t sarn_path_exists(const char* path)  { struct sarn_stat st; return (sarn_stat(path,&st)==0)?1:0; }
int32_t sarn_path_is_file(const char* path) { struct sarn_stat st; return (sarn_stat(path,&st)==0&&(st.st_mode&S_IFMT)==S_IFREG)?1:0; }
int32_t sarn_path_is_dir(const char* path)  { struct sarn_stat st; return (sarn_stat(path,&st)==0&&(st.st_mode&S_IFMT)==S_IFDIR)?1:0; }
char* sarn_path_normalize(const char* path) { return sarn_path_absolute(path); }
