...
#include <windows.h>
#include <direct.h>
#define sarn_stat  _stat
#define sarn_mkdir(p) _mkdir(p)
#else
#include <dirent.h>
#include <unistd.h>
#define sarn_stat  stat
#define sarn_mkdir(p) mkdir(p, 0755)
#endif
#define FS_MAX 64
static FILE* _h[FS_MAX];
static int   _hinit;
static void _fs_init(void) { if (!_hinit) { memset(_h, 0, sizeof(_h)); _hinit = 1; } }
char* sarn_fs_read_all(const char* path) {
...
int32_t sarn_fs_write(const char* path, const char* data) {
    FILE* f = fopen(path, "wb"); if (!f) return 0;
    size_t n = strlen(data); int32_t ok = (int32_t)(fwrite(data, 1, n, f) == n);
    fclose(f); return ok;
}
int32_t sarn_fs_append(const char* path, const char* data) {
    FILE* f = fopen(path, "ab"); if (!f) return 0;
    size_t n = strlen(data); int32_t ok = (int32_t)(fwrite(data, 1, n, f) == n);
    fclose(f); return ok;
}
int32_t sarn_fs_exists(const char* path) { struct sarn_stat st; return (sarn_stat(path, &st) == 0) ? 1 : 0; }
int32_t sarn_fs_delete(const char* path) { return (remove(path) == 0) ? 1 : 0; }
int32_t sarn_fs_mkdir(const char* path)  { return (sarn_mkdir(path) == 0) ? 1 : 0; }
int32_t sarn_fs_rename(const char* a, const char* b) { return (rename(a, b) == 0) ? 1 : 0; }
int64_t sarn_fs_size(const char* path) { struct sarn_stat st; return (sarn_stat(path, &st) == 0) ? (int64_t)st.st_size : -1; }
char* sarn_fs_listdir(const char* path) {
...
int32_t sarn_fs_writeh(int64_t h, const char* data) {
    if (h<0||h>=FS_MAX||!_h[h]) return 0; size_t n=strlen(data); return (int32_t)(fwrite(data,1,n,_h[h])==n);
}
int32_t sarn_fs_flush(int64_t h) { if(h<0||h>=FS_MAX||!_h[h]) return 0; return (fflush(_h[h])==0)?1:0; }
int32_t sarn_fs_copy(const char* src, const char* dst) {
#ifdef _WIN32
    return CopyFileA(src, dst, FALSE) ? 1 : 0;
#else
    char* d = sarn_fs_read_all(src); if (!d) return 0; int32_t ok = sarn_fs_write(dst, d); free(d); return ok;
#endif
}
